...
use crate::windows::common::INPUTMESH_EXTRA_INFO;
...
fn sim_mouse_event(flags: DWORD, data: DWORD, dx: LONG, dy: LONG) -> Result<(), SimulateError> {
    ...
    *inner_union = MOUSEINPUT {
        ...
        dwFlags: flags,
        time: 0,
        dwExtraInfo: INPUTMESH_EXTRA_INFO,
    };
    let mut input = [INPUT {
        type_: INPUT_MOUSE,
        u: union,
...
pub fn simulate(event_type: &EventType) -> Result<(), SimulateError> {
    match event_type {
        ...
        EventType::MouseMove { x, y } => {
            ...
            if width <= 1 || height <= 1 || !x.is_finite() || !y.is_finite() {
                return Err(SimulateError);
            }

            let normalized_x = (((*x).round() as i64 - i64::from(left)) * 65_535
                / i64::from(width - 1))
            .clamp(0, 65_535) as LONG;
            let normalized_y = (((*y).round() as i64 - i64::from(top)) * 65_535
                / i64::from(height - 1))
            .clamp(0, 65_535) as LONG;
