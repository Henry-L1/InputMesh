...
/// Private tag attached to InputMesh events sent through `SendInput`.
///
/// Windows exposes this value unchanged to the low-level mouse and keyboard
/// hooks.  Coordinate matching alone is not reliable on mixed-DPI desktops,
/// so the tag is the authoritative way to prevent injected events from being
/// treated as fresh physical input and sent back to the peer.
pub const INPUTMESH_EXTRA_INFO: usize = 0x494D_4553;
...
pub unsafe fn is_inputmesh_mouse_event(lpdata: LPARAM) -> bool {
    let mouse = *(lpdata as *const MSLLHOOKSTRUCT);
    mouse.dwExtraInfo == INPUTMESH_EXTRA_INFO
}

pub unsafe fn is_inputmesh_keyboard_event(lpdata: LPARAM) -> bool {
    let keyboard = *(lpdata as *const KBDLLHOOKSTRUCT);
    keyboard.dwExtraInfo == INPUTMESH_EXTRA_INFO
}

pub unsafe fn is_inputmesh_event(param: WPARAM, lpdata: LPARAM) -> bool {
    match param.try_into() {
        Ok(WM_KEYDOWN) | Ok(WM_SYSKEYDOWN) | Ok(WM_KEYUP) | Ok(WM_SYSKEYUP) => {
            is_inputmesh_keyboard_event(lpdata)
        }
        ...
        | Ok(WM_MOUSEMOVE) | Ok(WM_MOUSEWHEEL) | Ok(WM_MOUSEHWHEEL) => {
            is_inputmesh_mouse_event(lpdata)
        }
        _ => false,
    }
}
