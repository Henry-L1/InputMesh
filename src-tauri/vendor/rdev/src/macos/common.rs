...
pub unsafe fn convert(
    ...
    if let Some(event_type) = option_type {
        ...
        .then(|| {
            (
                cg_event.get_integer_value_field(EventField::MOUSE_EVENT_DELTA_X),
                cg_event.get_integer_value_field(EventField::MOUSE_EVENT_DELTA_Y),
            )
        });
        // InputMesh routes physical key codes and never reads Event::name.
        // Querying the current TIS input source from this event-tap thread
        // violates macOS 15's main-queue precondition and aborts the process
        // with _dispatch_assert_queue_fail. Keep the localized name empty.
        let name = None;
        return Some(Event {
            event_type,
            time: SystemTime::now(),
            name,
            relative_delta,
            is_synthetic: cg_event.get_integer_value_field(EventField::EVENT_SOURCE_USER_DATA)
