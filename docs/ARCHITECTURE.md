...
节点检测到非注入的物理指针输入时，可以发布一个新的控制权 claim。键盘不绑定到产生鼠标事件的物理电脑：每个键盘事件都按逻辑指针当前所在屏幕路由；目标电脑上的本地键盘直接作用于当前焦点，不抢走指针控制权，另一台电脑上的键盘则可以在不移动指针的情况下把键盘消息发到当前远端目标。鼠标、按键和滚轮等传统输入事件携带当前 claim；独立的 `KeyboardInput` 只携带目标 `screen_id` 与发送方序号，接收者同时校验可信连接和当前逻辑指针焦点。
